...
namespace UCRSuite
{
    ...
    public class DTW
    {
        ...
        private int bestk;
        private int wk;  /* index position of worst of the KNN */
        private int _wk;
        private double[] _kvec;
        private int[] _lvec; 
        ...
        public DTW(List<MDVector> data, int dimensions = 1, int k=1, float warpWindow = 0.05f) :
            this(dimensions, k,  warpWindow)
        {
        ...
        private double ucr_set_knn(double thatdist, int thatloc)
        {
            //See here: https://github.com/wpleasant/dtwKnn/blob/master/src/ucrDTW.c
            double thatbsf = 0.0;
            if (thatloc-lastloc > jumpsize)
            {
                _kvec[wk] = thatdist;
                _lvec[wk] = thatloc;
                _wk = wk;
            ...
            for (int thatk= 0; thatk<bestk; thatk++)
            {
                if (_kvec[thatk]> thatbsf)
                {
                    thatbsf = _kvec[thatk];
                    wk = thatk; 
                }
        ...
        public DTWResult warp(Query query)
        {
            ...
            return new DTWResult() { Locations = _lvec, Distances = _kvec};
